...
class PrestoRequest(object):
    ...
    def __init__(
            ...
            redirect_handler=redirect.GatewayRedirectHandler(),
    ...
    def http_headers(self):
        ...
        headers[constants.HEADER_ENCODING] = ""
    ...
    def __iter__(self):
        ...
        while not self._query.is_finished():
            ...
            for row in rows:
                ...
                yield self._decode_binary(row)

    def _decode_binary(self, row):
        ...
        if self._binary_decoder is None:
            assert self._query.columns
            self._binary_decoder = {}
            i = 0
            for col in self._query.columns:
                if col["type"].lower() == 'varbinary':
                    self._binary_decoder[i] = base64.b64decode
                elif col["type"].lower() == 'geometry':
                    self._binary_decoder[i] = _decode_geometry
                else:
                    pass
                i += 1

        for (index, decoder) in self._binary_decoder.items():
            row[index] = decoder(row[index])
        return row


def _decode_geometry(base64_string):
    wkb = base64.b64decode(base64_string)
    return gdal.ogr.CreateGeometryFromWkb(wkb)
