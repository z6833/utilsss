...
from rs_utils.shapes.shape import LayerData
...
def add_detection_item(crop_rs, crop_shp, output_dir_img, save_name, out_shape, feature,num,num2):
    # a = crop_rs.profile['transform'].a
    # b = crop_rs.profile['transform'].b
    # c = crop_rs.profile['transform'].c
    # d = crop_rs.profile['transform'].d
    # e = crop_rs.profile['transform'].e
    # f = crop_rs.profile['transform'].f
    # affine_matrix = [1 / a, b / a, d / a, 1 / e, -c / a, -f / e]
    ...
    for i, plg in enumerate(new_polygons):
        ...
        print("np.array(plg.bounds): ",np.array(plg.bounds))
